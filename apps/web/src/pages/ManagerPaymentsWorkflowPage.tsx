...
type BalancePeriod = PaymentPeriod & { pendingCents: number; pendingFormatted: string; verifiedCents: number; verifiedFormatted: string; payableCents: number; payableFormatted: string; blockedCents: number; blockedFormatted: string };
type Balance = { id: string; displayName: string; isActive: boolean; pendingCents: number; pendingFormatted: string; verifiedCents: number; verifiedFormatted: string; payableCents: number; payableFormatted: string; blockedCents: number; blockedFormatted: string; payableEarningIds: string[]; paymentPeriods?: BalancePeriod[] };
type Receipt = { id: string; originalName: string; mimeType: string; sizeBytes: number };
type Payment = { id: string; chatter: { displayName: string }; manager: { displayName: string }; totalFormatted: string; paidAt: string; paymentPeriods?: PaymentPeriod[]; receipt?: Receipt | null };
...
export const ManagerPaymentsWorkflowPage = () => {
  ...
  return <section className="stack-gap payments-page">
    ...
    <div className="card table-card"><h2>Histórico de pagamentos</h2>{loading ? <div className="skeleton-list" aria-label="Carregando histórico"><div className="skeleton" /><div className="skeleton" /></div> : <table><thead><tr><th>Data</th><th>Referência</th><th>Chatter</th><th>Valor</th><th>Gerente</th><th>Comprovante</th></tr></thead><tbody>{history.map((item) => <tr key={item.id}><td>{formatDateTime(item.paidAt)}</td><td>{item.paymentPeriods?.map((period) => <PaymentPeriodIndicator key={period.paymentDate} period={period} referenceOnly compact />) ?? "—"}</td><td>{item.chatter.displayName}</td><td>{item.totalFormatted}</td><td>{item.manager.displayName}</td><td>{item.receipt ? <ReceiptButton receipt={item.receipt} /> : "—"}</td></tr>)}</tbody></table>}{!loading && history.length === 0 ? <p className="empty-hint">Nenhum pagamento registrado.</p> : null}</div>

    <ModalDialog open={Boolean(payTarget)} onClose={closePaymentModal} ariaLabel="Confirmar pagamento" panelClassName="modal modal-solid payment-confirmation-modal">{payTarget ? <><h2>Confirmar pagamento</h2><p className="payment-confirmation-summary"><strong>{payTarget.payableFormatted}</strong> em {payTarget.payableEarningIds.length} horário(s) confirmado(s) por <strong>{payTarget.displayName}</strong>.</p>{payTarget.paymentPeriods?.length ? <div className="payment-period-list">{payTarget.paymentPeriods.filter((period) => period.payableCents > 0).map((period) => <PaymentPeriodIndicator key={period.paymentDate} period={period} amountFormatted={period.payableFormatted} />)}</div> : null}<label className="file-control payment-receipt-field"><span>Comprovante <small className="optional-label">Opcional</small></span><span className={`file-picker-shell${receiptFile ? " has-file" : ""}`}><span className="file-picker-action"><Upload size={15} /> Escolher arquivo</span><span className="file-picker-name" title={receiptFile?.name}>{receiptFile?.name ?? "Nenhum arquivo selecionado"}</span><input type="file" accept="application/pdf,image/png,image/jpeg,image/webp" aria-label="Selecionar comprovante do pagamento" onChange={(event) => setReceiptFile(event.target.files?.[0] ?? null)} /></span><small className="file-picker-hint">PDF, PNG, JPG ou WebP · até 10 MB</small></label><div className="modal-actions"><button className="secondary-button" onClick={closePaymentModal} disabled={paying}>Cancelar</button><button className="primary-button" disabled={paying} onClick={() => void pay()}>{paying ? "Registrando…" : "Confirmar pagamento"}</button></div></> : null}</ModalDialog>
    <ModalDialog open={Boolean(importDetail)} onClose={() => setImportDetail(null)} ariaLabel="Resultado da conferência" panelClassName="modal reconciliation-modal">{importDetail ? <><h2>Resultado da conferência</h2><p>{importDetail.modelTag.name} · {importDetail.vendorName}</p><div className="reconciliation-results">{importDetail.reconciliations.map((item) => <article className={`reconciliation-result status-${item.status.toLowerCase()}`} key={item.id}><div><strong>{item.shift.chatter.displayName}</strong><small>{formatDateTime(item.shift.startedAt)} — {formatDateTime(item.shift.endedAt)}</small></div><div><span>Informado {money(item.reportedGrossCents)}</span><span>Extrato {money(item.statementCommissionCents)}</span><strong>Diferença {money(item.deltaCents)}</strong></div><span className="status-badge">{item.status}</span>{item.status !== "MATCHED" && item.status !== "OVERRIDDEN" ? <button className="secondary-button" onClick={() => setOverrideTarget(item)}>Liberar com justificativa</button> : null}</article>)}</div></> : null}</ModalDialog>
